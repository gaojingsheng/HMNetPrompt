    ...
    def tokenize(rand: Random, doc):
        ...
        for sample in doc:
            ...
            if sample['task'] == 'sum':
                ...
                for topic in data['meeting']:
                    topic_segment+=[1]+[0]*(len(topic)-1)
                topic_segment[0] = 0
                print('topic_segment:', topic_segment)
                data['meeting'] = [n for a in data['meeting'] for n in a]
                print('data[meeting]:', data['meeting'])'''
                ...
                for turn in data['meeting']:
                    ...
                    sum_list = summarize(origin_text, ratio=0.3, split=True)
                    textrank_res.append(" ".join(sum_list))
                    ...
                    for i in range(len(sum_lists)):
                        ...
                        if tmp_index + len(sub_list) < len(turn["utt"]["word"]) - 1:
                            turn["utt"]["word"] = turn["utt"]["word"][:tmp_index] + [tokenizer.mask_token] + \
                                                  turn["utt"]["word"][tmp_index + len(sub_list) + 1:]
                        else:
                            turn["utt"]["word"] = turn["utt"]["word"][:tmp_index] + [tokenizer.mask_token]
                ...
                summary_str1 = ''
                for sub_sum in data['summary']:
                    sub_str = ' '.join(sub_sum)
                    summary_str1 += sub_str + ' '
                summary_str = ' '.join(textrank_res)
                    #if dataset_label == 'train' else summary_str1
                if anon_roles:
