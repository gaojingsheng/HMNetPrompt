...
def HMNetBatchGen(task_args, dataset_label, model_config=None, tokenizer=None, world_size=None, rank=None, seed=None):
    ...
    gsg_ratio = task_args.get('GSG_RATIO', 0.3)
    ...
    def tokenize(rand: Random, doc):
        ...
        for sample in doc:
            ...
            if sample['task'] == 'sum':
                ...
                for order, turn in enumerate(data['meeting']):
                    #origin_text = " ".join(turn["utt"]["word"])
                    if turn["topic"] == 1:
                        if one_topic_sentences != None:
                            all_split_sentences.append(one_topic_sentences)
                        one_topic_sentences = []
                    origin_list = make_all_split_sentences(turn["utt"]["word"])
                    one_topic_sentences.extend(origin_list)
                if one_topic_sentences != None:
                    all_split_sentences.append(one_topic_sentences)

                for one_topic_sentences in all_split_sentences:
                    #one_topic_rouge = []
                    one_topic_rouge_res = []
                    topic_index += 1
                    for i in range(len(one_topic_sentences)):
                        f1score = 0
                        for j in range(len(one_topic_sentences)):
                            if i==j:
                                continue
                            f1score += rouge_n(one_topic_sentences[i], one_topic_sentences[j], n=1)[0]
                        one_topic_rouge_res.append((one_topic_sentences[i], i, f1score))
                        sorted(one_topic_rouge_res, key=lambda x:x[2], reverse=True)
                        #print(type(one_topic_rouge_res))
                        one_topic_rouge_res = one_topic_rouge_res[:int(len(one_topic_sentences)*gsg_ratio)+1]
                        one_topic_rouge_res.sort(key=lambda x:x[1])
                        #print('one_topic_rouge_res:',one_topic_rouge_res)
                    for k in range(len(one_topic_rouge_res)):
                        splitted_sentences.append(one_topic_rouge_res[k][0].split(" "))
                        rouge_res.append(one_topic_rouge_res[k][0])

                #print('rouge_res:', rouge_res)

                        #temp = [one_topic_sentences[item] for item in len(one_topic_sentences) if item != index]
                ...
                for order, turn in enumerate(data['meeting']):
                    ...
                    if dataset_label != None:
                        for splitted_sentence in splitted_sentences[start_index:]:
                            tmp_index = 10000
                            max_index = len(turn["utt"]["word"]) - len(splitted_sentence) + 1
                            if max_index <= 0:
                                break
                            for l in range(max_index):
                                if turn["utt"]["word"][l:l + len(splitted_sentence)] == splitted_sentence:
                                    tmp_index = l
                                    if tmp_index + len(splitted_sentence) < len(turn["utt"]["word"]) - 1:
                                        #print('orign turn["utt"]["word"]:', turn["utt"]["word"])
                                        #print('tokenizer.mask_token:', tokenizer.mask_token)
                                        turn["utt"]["word"] = turn["utt"]["word"][:tmp_index] + [tokenizer.mask_token] + turn["utt"]["word"][tmp_index + len(splitted_sentence):]
                                        #print('add_mask:', add_mask)
                                        #print('turn["utt"]["word"]:', turn["utt"]["word"])
                                        turn["utt"]["pos_id"] = turn["utt"]["pos_id"][:tmp_index] + [0] + turn["utt"]["pos_id"][tmp_index + len(splitted_sentence):]
                                        turn["utt"]["ent_id"] = turn["utt"]["ent_id"][:tmp_index] + [0] + turn["utt"]["ent_id"][tmp_index + len(splitted_sentence):]
                                        # print('orign splitted_sentences:', splitted_sentences)
                                        #splitted_sentences.pop(0)

                                        # print('splitted_sentences:', splitted_sentences)
                                    else:
                                        #print('orign turn["utt"]["word"]:', turn["utt"]["word"])
                                        turn["utt"]["word"] = turn["utt"]["word"][:tmp_index] + [tokenizer.mask_token]
                                        #print('turn["utt"]["word"]:', turn["utt"]["word"])
                                        turn["utt"]["pos_id"] = turn["utt"]["pos_id"][:tmp_index] + [0]
                                        turn["utt"]["ent_id"] = turn["utt"]["ent_id"][:tmp_index] + [0]
                                        # print('orign splitted_sentences:', splitted_sentences)
                                        #splitted_sentences.pop(0)
                                        # print('splitted_sentences:', splitted_sentences)
                                    start_index += 1
                                    if start_index == len(splitted_sentences):
                                        break
                                else:
                                    continue
                ...
                sample['target'] = {}
                summary_str = ' '.join(rouge_res)
                #if dataset_label == 'train' else ' '.join(data['summary'])
                ...
                #print('sample[source][sequence]:', sample['source']['sequence'])
            else:
    ...
    def _pad_batch(batch):
        ...
        x_topic_tensor = torch.LongTensor(actual_size, x_topic_max_len).fill_(2)  # 这里需要额外注意一下，处理时需要去掉2
        ...
        for i in range(len(x_sent_batch)):
            for j in range(len(x_sent_batch[i])):
                ...
                x_mask_tensor[i, j, :len(x_sent_batch[i][j])] = 1
        ...
        # print('x_mask_tensor:',x_mask_tensor)

        return {
            'encoder_input_ids': x_tensor,
            'encoder_input_mask_ids': x_mask_tensor,
