    ...
    def tokenize(rand: Random, doc):
        ...
        for sample in doc:
            ...
            if sample['task'] == 'sum':
                ...
                source['sequence'] = []
                rouge_res = []
                print('len(data[summary]):',len(data['summary']))
                print(len(data['meeting']))
                ...
                for order, turn in enumerate(data['meeting']):
                    ...
                    if dataset_label != None:
                        ...
                        sum_list = []
                        for sum in data['summary'][order]:
                            # print('sum:',sum)
                            f1score = []
                            for i in range(len(origin_list)):
                                # print('origin_list[i]:',origin_list[i])
                                f1score.append(rouge_n(origin_list[i], sum, n=1)[0])
                            if len(origin_list) != 0:
                                sum_list.append(origin_list.pop(f1score.index(max(f1score))))
                        # sum_list = summarize(origin_text, ratio = 0.1, split=True)
                        # print('sum_list:',sum_list)
                        rouge_res.append(" ".join(sum_list))
                        sum_lists = []
                        for m in range(len(sum_list)):
                            sum_lists.append(sum_list[m].replace("\n", "").split())
                        # sum_lists = list(filter(None, re.split(r'[.?!]',sum_text)))
                        # print('sum_lists:',sum_lists)
                        for i in range(len(sum_lists)):
                            sub_list = sum_lists[i]
                            tmp_index = 10000
                            max_index = len(turn["utt"]["word"]) - len(sub_list) + 1
                            for l in range(max_index):
                                if turn["utt"]["word"][l:l + len(sub_list)] == sub_list:
                                    tmp_index = l
                                    continue
                            if tmp_index+len(sub_list)<len(turn["utt"]["word"])-1:
                                turn["utt"]["word"] = turn["utt"]["word"][:tmp_index] + [tokenizer.mask_token] + turn["utt"]["word"][tmp_index + len(sub_list) + 1:]
                            else:
                                turn["utt"]["word"] = turn["utt"]["word"][:tmp_index] + [tokenizer.mask_token]
                        #print('res_str:',turn["utt"]["word"])
