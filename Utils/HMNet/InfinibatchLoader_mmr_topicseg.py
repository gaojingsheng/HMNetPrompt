...
def HMNetBatchGen(task_args, dataset_label, model_config=None, tokenizer=None, world_size=None, rank=None, seed=None):
    ...
    max_gen_length = task_args.get('TRAIN_MAX_GEN_LENGTH', 200)
        ...
        for sample in doc:
            ...
            if sample['task'] == 'sum':
                ...
                for one_topic_sentences in all_split_sentences:
                    one_topic_text = " ".join(one_topic_sentences)
                    sum_list = summarize(one_topic_text, ratio=0.1, split=True, use_mmr=True)
                    rouge_res.extend(sum_list)
