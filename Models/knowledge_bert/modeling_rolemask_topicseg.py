...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask):
        ...
        #print('attention_mask_233:', attention_mask)
...
class BertLayerMix(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, hidden_states_ent, attention_mask_ent, ent_mask):
        ...
        # print('attention_output_ent:', attention_output_ent.size())
...
class BertPredictionHeadTransform(nn.Module):
    def __init__(self, config):
        ...
        self.dense = nn.Linear(544, config.hidden_size)#修改提醒：这里必须使用数字，使用config.hidden_size会报错
        self.transform_act_fn = ACT2FN[config.hidden_act] \
            if isinstance(config.hidden_act, str) else config.hidden_act
        self.LayerNorm = BertLayerNorm(config.hidden_size, eps=1e-12)

    def forward(self, hidden_states):
       # print('hidden_states_head:',hidden_states.size())
        hidden_states = self.dense(hidden_states)
...
class BertPreTrainingHeads(nn.Module):
    ...
    def forward(self, sequence_output, candidate):
        prediction_scores_ent = self.predictions_ent(sequence_output, candidate)
        return prediction_scores_ent
...
class BertModel(PreTrainedBertModel):
    ...
    def forward(self, input_embedding, token_type_ids=None, attention_mask=None, input_ent=None, ent_mask=None, output_all_encoded_layers=True):
        if attention_mask is None:
            attention_mask = torch.ones(input_embedding.size(0),input_embedding.size(1)).to(input_embedding.device)
            extended_attention_mask = attention_mask.unsqueeze(1).unsqueeze(2)
        else:
            extended_attention_mask = attention_mask.unsqueeze(1)

        if ent_mask is None:
            ent_mask = torch.ones(input_ent.size(0),input_ent.size(1)).to(input_embedding.device)
        #if token_type_ids is None:
            #token_type_ids = torch.zeros_like(input_ids)
        ...
        #print('attention_mask_orign:', attention_mask)
        ...
        encoded_layers = self.encoder(input_embedding,
...
class BertForPreTraining(PreTrainedBertModel):
    ...
    def forward(self, input_embedding, token_type_ids=None, attention_mask=None,
        input_ent=None, ent_mask=None, candidate=None, ent_labels=None):
        # the id in ent_labels should be consistent with the order of candidate.
        sequence_output, pooled_output = self.bert(input_embedding, token_type_ids, attention_mask, input_ent, ent_mask,
                                                   output_all_encoded_layers=False)
        # candidate [entity_num_in_the_batch, dim]
        # prediction_scores_ent [batch_size, max_seq, entity_num_in_the_batch]
        prediction_scores_ent = self.cls(sequence_output, candidate)

        if ent_labels is not None:
            loss_fct = CrossEntropyLoss(ignore_index=-1)
            ent_ae_loss = loss_fct(prediction_scores_ent.view(-1, candidate.size()[0]), ent_labels.view(-1))
            return sequence_output, ent_ae_loss
        else:
            return sequence_output, prediction_scores_ent
