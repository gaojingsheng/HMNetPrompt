...
from Models.knowledge_bert.modeling_rolemask_topicseg import BertForPreTraining, BertConfig
...
class MeetingNet_Transformer_multitask(nn.Module):
    def __init__(self, opt):
        super(MeetingNet_Transformer_multitask, self).__init__()
        ...
        if 'USE_SEGMENT' in self.opt:
            self.encoder = Encoder_topicseg(opt, self.vocab_size, vocab_dim, role_dim, embedder, role_embed)
        else:
    ...
    def _forward(self, **kwargs):
        ...
        encoder_input_mask = kwargs.pop('encoder_mask_ids') if 'USE_PRETRAIN' in self.opt else None
        encoder_input_topics = kwargs.pop('encoder_input_topics') if 'USE_PRETRAIN' in self.opt else None
        ...
        if 'USE_SEGMENT' in self.opt:
            token_encoder_outputs, sent_encoder_outputs, topic_segment_output, role_ae_loss = self.encoder(encoder_input_ids, encoder_input_mask, encoder_input_roles, encoder_input_pos, encoder_input_ent, encoder_input_topics)
        else:
            token_encoder_outputs, sent_encoder_outputs, topic_segment_output, role_ae_loss = self.encoder(encoder_input_ids, encoder_input_mask, encoder_input_roles, encoder_input_pos, encoder_input_ent)
        vocab_logprob = self.decoder(token_encoder_outputs, sent_encoder_outputs, decoder_input_ids)
        return vocab_logprob, (token_encoder_outputs, sent_encoder_outputs), topic_segment_output, role_ae_loss
    ...
    def generate(self, batch, max_sent_len):
        ...
        input_mask = batch['encoder_mask_ids'] if 'USE_PRETRAIN' in self.opt else None
        input_topics = batch['encoder_input_topics'] if 'USE_PRETRAIN' in self.opt else None
        ...
        outputs = self._generate(
            input_ids=input_ids, input_mask=input_mask, input_roles=input_roles,
            input_pos=input_pos, input_ent=input_ent, input_topics=input_topics,
    ...
    def _generate(
        ...
        if 'USE_SEGMENT' in self.opt:
            encoder_outputs = self.encoder(input_ids, input_mask, input_roles, input_pos, input_ent, input_topics)
        else:
            encoder_outputs = self.encoder(input_ids, input_mask, input_roles, input_pos, input_ent)
...
class TurnLevelEncoder(nn.Module):
    ...
    '''
      ...
      Output:
        h: batch x len x n_state
    '''
    def forward(self, x, x_role, role_embed, topic_segment):
       ...
       # print('x_role:',x_role)
        ...
        attention_mask = torch.zeros(x.size(0), x.size(1), x.size(1)).to(x.device)
        '''topic_segment_id = topic_segment.cpu().numpy().tolist()
        topic_segment_index=[]
        for topic in topic_segment_id:
            temp=[]
            for j in range(len(topic)):
                if topic[j]==1:
                    temp.append(j)
            temp.append(len(topic)-1)
        topic_segment_index.append(temp)'''
        
        for i in range(x.size(0)):
            left,right = x.size(1)-1,x.size(1)-1
            for j in range(x.size(1)-1,-1,-1):
                if topic_segment[i][j] == 1:
                    right = left
                    left = j
                    attention_mask[i,left:right,left:right] = 1 
                             
        sequence_output, role_ae_loss = self.turn_level_encoder(input_embedding=x, attention_mask=attention_mask ,input_ent=ent_emb, ent_mask=mask, candidate=ent_candidate, ent_labels=ent_labels)
...
class TopicSegment(nn.Module):
    ...
    def forward(self, x):
        ...
        return preds_topicseg, logits

class Encoder_topicseg(nn.Module):
    '''
    ...
    def __init__(self, opt, vocab_size, embed_size, role_dim, embedder, role_embed):
        super(Encoder_topicseg, self).__init__()
        self.opt = opt
    ...
    '''
    def forward(self, x, x_mask, x_role, x_pos, x_ent, topic_segment_id=None):
        batch_size = x.size(0)
        sent_num = x.size(1)
        x_len = x.size(2)
        ...
        if topic_segment_id == None:
            topic_segment_id = topic_segment_output[0]
        #sent_encoder_outputs = self.sent_encoder(sent_encoder_inputs) # 1 x sent_num x sent_transformer_dim
        sent_encoder_outputs, role_ae_loss = self.sent_encoder(sent_encoder_inputs, x_role, self.role_embed, topic_segment_id)

        token_transformer_output = token_transformer_output.view(batch_size, -1, token_transformer_dim)

        return token_transformer_output, sent_encoder_outputs, topic_segment_output[1], role_ae_loss
...
class Encoder(nn.Module):
    ...
    '''
     ...
     outputs:
       ...
       sent_encoder_outputs:  bz x sent_num x sent_transformer_dim
    '''
    def forward(self, x, x_mask, x_role, x_pos, x_ent):
        batch_size = x.size(0)
        sent_num = x.size(1)
        x_len = x.size(2)
        ...
        return token_transformer_output, sent_encoder_outputs, topic_segment_output, role_ae_loss
