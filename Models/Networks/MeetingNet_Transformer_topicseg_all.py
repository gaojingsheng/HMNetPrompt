...
class MeetingNet_Transformer_topicseg_all(nn.Module):
    def __init__(self, opt):
        super(MeetingNet_Transformer_topicseg_all, self).__init__()
    ...
    def _forward(self, **kwargs):
        ...
        token_encoder_outputs, sent_encoder_outputs = self.encoder(encoder_input_ids, encoder_input_roles, encoder_input_pos, encoder_input_ent, encoder_input_topics)
    ...
    def _generate(
        ...
        encoder_outputs = self.encoder(input_ids, input_roles, input_pos, input_ent, input_topics)
...
class Encoder(nn.Module):
    ...
    def segmentemb(self, seg_id):
        ...
        seg_id[:, 0] = 0
        #print('seg_id:',seg_id)
        #return seg_id.to(cuda_device)
        return seg_id

    def forward(self, x, x_role, x_pos, x_ent, x_topic):
        batch_size = x.size(0)
        sent_num = x.size(1)
        x_len = x.size(2)
        ...
        sent_encoder_outputs = self.sent_encoder(sent_encoder_inputs, x_topic) # 1 x sent_num x sent_transformer_dim
