...
class MeetingNet_Transformer_ernie_topicseg_cpu(nn.Module):
    def __init__(self, opt):
        super(MeetingNet_Transformer_ernie_topicseg_cpu, self).__init__()
    ...
    def forward(self, batch, beam_search=False, max_sent_len=None):
        ...
        return vocab_logprob, role_ae_loss
    ...
    def _forward(self, **kwargs):
        ...
        token_encoder_outputs, sent_encoder_outputs, role_ae_loss = self.encoder(encoder_input_ids, encoder_input_roles, encoder_input_pos, encoder_input_ent, encoder_input_topics)
        vocab_logprob = self.decoder(token_encoder_outputs, sent_encoder_outputs, decoder_input_ids)
        return vocab_logprob, (token_encoder_outputs, sent_encoder_outputs), role_ae_loss
...
class Encoder(nn.Module):
    ...
    def forward(self, x, x_role, x_pos, x_ent, x_topic):
        ...
        if self.opt['TYPE_POOLING'] == 'CLS':
            sent_encoder_inputs = token_transformer_output[:, :, 0, :] # 1 x sent_num x token_transformer_dim
        elif self.opt['TYPE_POOLING'] == 'MEAN':
            sent_encoder_inputs = torch.mean(input=token_transformer_output, dim=2, keepdim=False)
        elif self.opt['TYPE_POOLING'] == 'MAX':
            sent_encoder_inputs = torch.max(input=token_transformer_output, dim=2)[0]
        else:
            raise Exception("Sorry, TYPE_POOLING is wrong!")
