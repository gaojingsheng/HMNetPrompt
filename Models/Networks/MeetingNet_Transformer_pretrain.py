...
class MeetingNet_Transformer_pretrain(nn.Module):
    def __init__(self, opt):
        super(MeetingNet_Transformer_pretrain, self).__init__()
        ...
        #self.encoder_eval = Encoder_eval(opt, self.vocab_size, vocab_dim, role_dim, embedder, role_embed)
        ...
        #self.decoder_eval = Decoder(opt, vocab_dim, self.vocab_size, embedder, self.encoder_eval.token_transformer_dim,self.encoder_eval.sent_transformer_dim)
    ...
    def _forward(self, **kwargs):
        ...
        encoder_input_mask = kwargs.pop('encoder_mask_ids')
        ...
        token_encoder_outputs, sent_encoder_outputs = self.encoder(encoder_input_ids, encoder_input_mask, encoder_input_roles, encoder_input_pos, encoder_input_ent)
...
class MeetingTransformerEncoder_PRETRAIN(nn.Module):
    '''
      Input:
        transformer_embed_dim: transformer dimension
    '''
    def __init__(self, opt, transformer_embed_dim):
        super(MeetingTransformerEncoder_PRETRAIN, self).__init__()
        vocab = int(opt['vocab_size'])
        n_layer = int(opt['TRANSFORMER_LAYER'])
        opt['transformer_embed_dim'] = transformer_embed_dim
        block = EncoderBlock(opt)
...
class Encoderfortokenlevel(nn.Module):
    '''
      Input:
        transformer_embed_dim: transformer dimension
    '''
    def __init__(self, opt, transformer_embed_dim):
        super(Encoderfortokenlevel, self).__init__()
        vocab = int(opt['vocab_size'])
        n_layer = int(opt['TRANSFORMER_LAYER'])
        opt['transformer_embed_dim'] = transformer_embed_dim
        block = EncoderBlock(opt)
...
class Encoder(nn.Module):
    ...
    def __init__(self, opt, vocab_size, embed_size, role_dim, embedder, role_embed):
        ...
        self.token_encoder = MeetingTransformerEncoder_PRETRAIN(opt, self.token_transformer_dim)
    ...
    def forward(self, x, x_mask, x_role, x_pos, x_ent):
        ...
        '''previous_token_transformer_output = self.previous_token_encoder(embedded.view(-1, x_len, feat_dim))
        previous_token_transformer_dim = previous_token_transformer_output.size(2)
        previous_token_transformer_output = previous_token_transformer_output.view(batch_size, sent_num, x_len, token_transformer_dim)
        # 1 x sent_num x x_len x token_transformer_dim'''

        sent_encoder_inputs = token_transformer_output[:, :, 0, :] # 1 x sent_num x token_transformer_dim
        #sent_encoder_inputs_0 = torch.mean(input=token_transformer_output,dim=2,keepdim=False)
        if 'USE_ROLE' in self.opt:
