...
'''
  TurnLeverEmbedder
'''
class TurnLeverEmbedder(nn.Module):
    '''
      Input:
        vocab: size of vocabulary
    '''
    def __init__(self, opt, embed=None):
        super(TurnLeverEmbedder, self).__init__()
        if 'USE_POSENT' in opt:
            n_state = int(opt['transformer_embed_dim']) + int(opt['POS_DIM']) + int(opt['ENT_DIM'])# n_state
        else:
        ...
        self.drop = nn.Dropout(embed_dropout_rate)
        self.pos_emb = PositionalEmbedding(opt, n_state)
        '''if 'USE_SEGMENT' in opt:
            self.seg_emb = nn.Embedding(2, n_state)'''
    ...
    '''
       ...
       Output:
        h: batch x len x n_state
    '''
    def forward(self, x, seg_id=None):
        #x_emb = self.embed(x)
        batch_size = x.shape[0]
        x_len = x.shape[1]
        pos_id = torch.arange(x_len).type(torch.cuda.FloatTensor)
        x_pos = self.pos_emb(pos_id)  # len x n_state
        x_pos = Variable(x_pos.unsqueeze(0).repeat(batch_size, 1, 1), requires_grad=False).cuda()
            ...
            for i in range(len(seg_id)):
                ...
                for j in range(1,len(seg_id[0])):
                    ...
                    if flag:
                        ...
                        seg_id[i][j]=0
            seg_id = torch.LongTensor(seg_id).to(x.device)
            seg_id[:,0]=0
            #print('seg_id:',seg_id)
            x_seg = self.seg_emb(seg_id)
            x_pos = x_pos + x_seg
        x_input = x + x_pos
        h = self.drop(x_input)
        return h
