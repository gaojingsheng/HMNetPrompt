...
class MeetingNet_Transformer_ernie(nn.Module):
    def __init__(self, opt):
        super(MeetingNet_Transformer_ernie, self).__init__()
        ...
        special_tokens_tuple_list = [("eos_token", 128), ("unk_token", 129), ("pad_token", 130), ("bos_token", 131), ("mask_token", 132), ("sep_token", 133)]
    ...
    def _forward(self, **kwargs):
        ...
        encoder_input_topics = kwargs.pop('encoder_input_topics') if 'USE_TOPICSEG' in self.opt else None
        ...
        token_encoder_outputs, sent_encoder_outputs = self.encoder(encoder_input_ids, encoder_input_roles, encoder_input_pos, encoder_input_ent, x_topicseg=encoder_input_topics)
    ...
    def generate(self, batch, max_sent_len):
        ...
        input_topics = batch["encoder_input_topics"] if 'USE_TOPICSEG' in self.opt else None
        ...
        outputs = self._generate(
            input_ids=input_ids, input_roles=input_roles, input_topics=input_topics,
    ...
    def _generate(
        ...
        encoder_outputs = self.encoder(input_ids, input_roles, input_pos, input_ent, x_topicseg=input_topics)
...
class Encoder(nn.Module):
    ...
    '''
    def forward(self, x, x_role, x_pos, x_ent, x_topicseg=None):
        batch_size = x.size(0)
        sent_num = x.size(1)
        x_len = x.size(2)
        ...
        #sent_encoder_inputs = torch.mean(input=token_transformer_output,dim=2,keepdim=False)
        if 'USE_ROLE' in self.opt:
            sent_encoder_inputs = torch.cat([sent_encoder_inputs, role_embed], dim=2)
        sent_encoder_outputs = self.sent_encoder(sent_encoder_inputs, x_topicseg=x_topicseg) # 1 x sent_num x sent_transformer_dim
