    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch_idx, numEpochs):
            ...
            for batch_idx, batch in enumerate(self.train_batch_generator):
                ...
                if ('SAVE_PER_UPDATE_NUM' in self.opt) and (self.updates + 1) % self.opt['SAVE_PER_UPDATE_NUM'] == 0: #and self.train_loss.val <= 4.00:
                ...
                if ('SHOW_PER_UPDATE_NUM' in self.opt) and (self.updates + 1) % self.opt['SHOW_PER_UPDATE_NUM'] == 0: #and self.train_loss.val <= 4.00:
                ...
                if show_eval_results:
                    ...
                    with torch.no_grad():
                        for j, eval_batch in enumerate(batches):
                            ...
                            vocab_logprob = self.module(eval_batch)[0]
                ...
                if save_a_checkpoint:
                    ...
                    if self.task.evaluator is not None:
                        ...
                        if got_better_score:
                            ...
                            self.best_update = self.updates
                        self.log("Updates {0} - {1}: Current Score: {2:.3f} (best Score: {3:.3f}, at updates {4})".format(
                            self.updates, eval_dataset, score, self.task.evaluator.best_score, self.updates, self.best_update))
                        self.log(
