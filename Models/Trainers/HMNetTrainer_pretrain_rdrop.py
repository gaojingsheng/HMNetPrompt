...
class WrappedModel(nn.Module):
    ...
    def KL(self, input, target, reduction="none"):
        input = input.float()
        target = target.float()
    ...
    def forward(self, batch):
        #添加正则化
        '''l1_regularization, l2_regularization = torch.tensor(0, dtype=torch.float32, requires_grad=True).to('cuda'), torch.tensor(0,dtype=torch.float32, requires_grad=True).to('cuda')  # 定义L1及L2正则化损失
        for param in self.model.parameters():
            l1_regularization += torch.norm(param, 1)
            #l1_regularization += torch.sum(torch.abs(param))
        l1_regularization=l1_regularization.type(torch.long)
        print('l1_regular:',l1_regularization)'''
        output1, logit1 = self.model(batch)
        output2, logit2 = self.model(batch)
        kl_loss = self.KL(logit1, logit2, reduction="batchmean") + self.KL(logit2, logit1, reduction="batchmean")
        print('kl_loss:', kl_loss.detach())
        loss = (self.criterion(output1, batch) + self.criterion(output2, batch)) / 2 + kl_loss
        #print(loss)
        return loss


class HMNetTrainer_pretrain_rdrop(DistributedTrainer):
    '''
    ...
    def __init__(self, opt):
        super().__init__(opt)
        print("-----------using HMNetTrainer_pretrain_rdrop-----------------")
    ...
    def train(self):
        ...
        save_a_checkpoint, show_eval_results = False, False
        for epoch in range(self.start_epoch_idx, numEpochs):
            ...
            for batch_idx, batch in enumerate(self.train_batch_generator):
                ...
                if ('SAVE_PER_UPDATE_NUM' in self.opt) and (self.updates + 1) % self.opt['SAVE_PER_UPDATE_NUM'] == 0 and self.updates >=5000 :
                ...
                if ('SHOW_PER_UPDATE_NUM' in self.opt) and (self.updates + 1) % self.opt['SHOW_PER_UPDATE_NUM'] == 0 :
                    show_eval_results = True
                ...
                if show_eval_results:
                    ...
                    eval_loss, eval_counts = 0, 0
                    with torch.no_grad():
                        for j, eval_batch in enumerate(batches):
                            for b in eval_batch:
                                if torch.is_tensor(batch[b]):
                                    eval_batch[b] = eval_batch[b].to(self.opt['device'])

                            vocab_logprob = self.module(eval_batch)
                            eval_loss += self.criterion(vocab_logprob, eval_batch)
                            eval_counts += 1
                        self.tb_log_scalar('Eval/loss', eval_loss / eval_counts, self.updates)
                        self.log('Eval loss:[{0:.5f}]'.format(eval_loss / eval_counts))
                ...
                if (batch_idx % 10 == 0) or (epoch == 0 and batch_idx <= 50) or "DEBUG" in self.opt:
                    if self.opt['rank'] == 0:
                        ...
                        self.log('epochs[{0:6}] updates[{1:6}] bsz[{2:d}] train loss[{3:.5f}] avg train loss[{4:.5f}] learning rate[{5:.5e}] remaining[{6}]'.format(
                            epoch, self.updates, batch_size, self.train_loss.val, self.train_loss.avg, self.lr_scheduler.get_last_lr()[0],
                        ...
                        self.tb_log_scalar('Learning Rate/lr', self.lr_scheduler.get_last_lr()[0], self.updates)#这里本来是get_lr()
