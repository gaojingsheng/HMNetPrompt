...
class WrappedModel(nn.Module):
    def __init__(self, model, criterion, topicseg_criterion):
        super(WrappedModel, self).__init__()
        self.add_module('model', model)
        self.add_module('criterion', criterion)
        #self.add_module('topicseg_criterion', topicseg_criterion)

    def forward(self, batch):
        output = self.model(batch)
        loss = self.criterion(output[0], batch)
        '''
        #loss.retain_grad()
        #print('loss.grad:',loss.grad)
        topicseg_loss = self.topicseg_criterion(output[1], batch)
        role_ae_loss = output[2]
        print('topicseg_loss:', topicseg_loss)
        print('role_ae_loss', role_ae_loss)
        print('loss',loss)
        #loss = loss + role_ae_loss + topicseg_loss
        loss = topicseg_loss
        '''
        return loss
...
class HMNetTrainer(DistributedTrainer):
    ...
    def set_up_model(self):
        ...
        try:
            ...
            topicseg_criterion_class = getattr(criterion_module, 'FocalLoss')
            self.criterion = criterion_class(self.opt, self.module)
            self.topicseg_criterion = topicseg_criterion_class()
        except Exception as e:
            self.log(e)
    ...
    def initialize_fp16_DDP(self):
        ...
        '''
        self.network = WrappedModel(self.module, self.criterion, self.topicseg_criterion)
