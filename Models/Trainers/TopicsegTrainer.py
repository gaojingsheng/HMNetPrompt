...
class WrappedModel(nn.Module):
    ...
    def forward(self, batch):
        '''
        loss1, logit1 = self.model(batch)
        loss2, logit2 = self.model(batch)
        kl_loss = self.KL(logit1, logit2, reduction="batchmean") + self.KL(logit2, logit1, reduction="batchmean")
        #print('kl_loss:', kl_loss.detach())
        loss = (loss1[0] + loss2[0]) / 2 + kl_loss
        '''

        loss = self.model(batch)[0][0]
        return loss


class TopicsegTrainer(DistributedTrainer):
    '''
                ...
                if show_eval_results:
                    ...
                    with torch.no_grad():
                        for j, eval_batch in enumerate(batches):
                            ...
                            eval_loss += self.module(eval_batch)[0]
                            eval_counts += 1
